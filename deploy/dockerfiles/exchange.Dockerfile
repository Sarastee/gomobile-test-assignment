...
COPY --from=builder /build/deploy/scripts/prod-exchange-service-start.sh $APP_DIR/prod-exchange-service-start.sh
COPY --from=builder /build/deploy/env/.env.prod $APP_DIR//deploy/env/.env.prod

RUN setfacl -R -m u:web:rwx $APP_DIR/prod-exchange-service-start.sh

USER web

ENTRYPOINT ["bash", "prod-exchange-service-start.sh"]